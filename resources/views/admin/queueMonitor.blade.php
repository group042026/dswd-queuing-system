                                    ...
                                    <td>
                                        @if(!in_array($queue->queue_status, ['Completed', 'Cancelled', 'Abondoned']))
                                            <x-danger-button
                                                type="button"
                                                x-data=""
                                                x-on:click="$dispatch('open-modal', 'cancel-queue-modal-{{ $queue->id }}')"
                                        ...
                                        @elseif($queue->queue_status === 'Cancelled')
                                            <x-secondary-button
                                                type="button"
                                                x-data=""
                                                x-on:click="$dispatch('open-modal', 'view-cancellation-modal-{{ $queue->id }}')"
                                            >
                                                {{ __('View') }}
                                            </x-secondary-button>
                                        @else
                ...
                <div data-modal-container>
                    @foreach($queues as $queue)
                            ...
                            <x-modal name="cancel-queue-modal-{{ $queue->id }}" focusable>
                                <div class="p-6">
                                    ...
                                    <p class="text-sm text-gray-600 mb-6">
                                        {{ __('Are you sure you want to cancel queue') }}
                                    ...
                                    <form method="POST" action="{{ route('admin.queue.cancel', $queue->id) }}">
                                        ...
                                        <div class="mb-6">
                                            <label
                                                for="cancellation_reason_{{ $queue->id }}"
                                            ...
                                            <textarea
                                                id="cancellation_reason_{{ $queue->id }}"
                                                name="cancellation_reason"
                                                rows="4"
                                                maxlength="1000"
                                                required
                                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                                placeholder="{{ __('Enter the reason for cancelling this queue...') }}"
                                            ></textarea>
                                        </div>
                                        ...
                                        <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                                            <x-secondary-button
                                                type="button"
                                                x-on:click="$dispatch('close-modal', 'cancel-queue-modal-{{ $queue->id }}')"
                                            >
                                                {{ __('Close') }}
                                            </x-secondary-button>
                        ...
                        @if($queue->queue_status === 'Cancelled')
                            <x-modal name="view-cancellation-modal-{{ $queue->id }}" focusable>
                                <div class="p-6">
                                    <h2 class="text-lg font-bold text-gray-900 mb-4">
                                    ...
                                    <div class="rounded-lg border border-red-100 bg-red-50 p-4 text-sm text-gray-700 whitespace-pre-line">
                                        {{ $queue->cancellation_reason ?: __('No cancellation reason recorded.') }}
                                    </div>

                                    <div class="flex justify-end mt-6 pt-4 border-t border-gray-100">
                                        <x-secondary-button
                                            type="button"
                                            x-on:click="$dispatch('close-modal', 'view-cancellation-modal-{{ $queue->id }}')"
                                        >
                                            {{ __('Close') }}
                                        </x-secondary-button>
                                    </div>
        ...
        function renderQueueTable(queues) {
            ...
            queues.forEach(q => {
                ...
                const cancelCell = q.can_cancel
                    ? `<button
                            type="button"
                            class="inline-flex items-center gap-1.5 px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 transition"
                            style="border-radius: 8px;"
                            x-data=""
                    ...
                    : q.queue_status === 'Cancelled'
                        ? `<button
                                ...
                                x-data=""
                                x-on:click="$dispatch('open-modal', 'view-cancellation-modal-${q.id}')"
                        >
                                View
                        </button>`
                        : `<span class="text-gray-300 text-xs">—</span>`;
                ...
                if (q.can_cancel) {
                    modals += `
                        <div
                            ...
                            style="display: none;"
                        >
                            <div class="fixed inset-0 bg-gray-500 opacity-75" x-on:click="show = false"></div>

                            <div class="relative mb-6 bg-white rounded-lg shadow-xl sm:w-full sm:max-w-2xl sm:mx-auto">
                                <div class="p-6">
                                    <h2 class="text-lg font-bold text-gray-900 mb-4">
                                        Cancel Queue Entry
                                    </h2>
                                    ...
                                    <p class="text-sm text-gray-600 mb-6">
                                        Queue #${q.queue_number} will be cancelled.
                                    </p>
                                    ...
                                    <form method="POST" action="${q.cancel_url}">
                                        ...
                                        <textarea
                                            ...
                                            required
                                            class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                            placeholder="Enter the reason for cancelling this queue..."
                                        ></textarea>
                                        ...
                                        <div class="flex justify-end gap-3 pt-4 mt-6 border-t border-gray-100">
                                            ...
                                            <button
                                                type="submit"
                                                class="px-4 py-2 bg-red-600 text-white rounded-md font-semibold text-xs uppercase"
                                            >
                                                Cancel Queue
                                            </button>
                                        </div>
                ...
                if (q.queue_status === 'Cancelled') {
                    modals += `
                        <div
                            x-data="{ show: false }"
                            x-show="show"
                            x-on:open-modal.window="$event.detail == 'view-cancellation-modal-${q.id}' ? show = true : null"
                            x-on:close-modal.window="$event.detail == 'view-cancellation-modal-${q.id}' ? show = false : null"
                            class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50"
                            style="display: none;"
                        >
                            <div
                            ...
                            <div class="relative mb-6 bg-white rounded-lg shadow-xl sm:w-full sm:max-w-2xl sm:mx-auto">
                                <div class="p-6">
                                    ...
                                    <div class="rounded-lg border border-red-100 bg-red-50 p-4 text-sm text-gray-700 whitespace-pre-line">
                                        ${q.cancellation_reason || 'No cancellation reason recorded.'}
                                    </div>
