<x-approving-officer-layout>
    <style>
        ...
        .rev-banner__stripe--blue { background-color: var(--dswd-blue); }
        .rev-banner__stripe--yellow { background-color: var(--dswd-yellow); }
        .rev-banner__stripe--red { background-color: var(--dswd-red); }
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="rev-banner">
                <div class="rev-banner__content">
                    <div class="rev-banner__badge">DSWD Approving Officer Portal</div>
                    <h1 class="rev-banner__title">Review and Approval</h1>
                    <p class="rev-banner__description">
                        Assess evaluations completed by Social Workers, review remarks, eligibility justifications, and authorize the final release of program packages.
                    </p>
                </div>
            ...
            <div class="filter-card">
                <form method="GET" action="{{ route('approving-officer.review') }}" class="flex items-end gap-3 flex-wrap">
            ...
            <div class="data-card">
                <div class="mb-6 flex justify-between items-center border-b pb-4">
                    <div>
                        <h3 class="text-lg font-extrabold text-gray-800">{{ __('Pending Review List') }}</h3>
                        ...
                        <tbody>
                            ...
                            @forelse($pendingReview as $item)
                                <tr>
                                    ...
                                    <td>
                                        @if($isToday)
                                            <x-primary-button x-on:click="$dispatch('open-modal', 'review-modal-{{ $item->id }}')" class="btn-primary">
                                                {{ __('Review') }}
                                            </x-primary-button>
                                        @else
                                ...
                                <x-modal name="review-modal-{{ $item->id }}" maxWidth="2xl" :show="$errors->any()">
                                    <div class="p-6">
                                        ...
                                        <div class="mb-6 bg-slate-50 p-4 rounded-xl text-sm border border-slate-100 flex flex-col gap-4">
                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    <span class="text-xs text-gray-400 block uppercase font-bold">{{ __('Interview Date') }}</span>
                                                    <span class="font-semibold text-gray-700">{{ \Carbon\Carbon::parse($assessment->interview_date)->format('M d, Y') }}</span>
                                                </div>
                                                <div>
                                                    <span class="text-xs text-gray-400 block uppercase font-bold">{{ __('Means of Verification') }}</span>
                                                    <span class="font-semibold text-gray-700">{{ $assessment->means_verification }}</span>
                                                </div>
                                            </div>
                                            <div class="border-t pt-3">
                                                <span class="text-xs text-gray-400 block uppercase font-bold mb-1">{{ __('Assessment Findings') }}</span>
                                                <span class="font-semibold text-gray-700 leading-normal block">{{ $assessment->assessment_findings }}</span>
                                            </div>
                                            <div class="border-t pt-3">
                                                <span class="text-xs text-gray-400 block uppercase font-bold mb-1">{{ __('Recommendation') }}</span>
                                                <span class="font-semibold text-gray-700 leading-normal block">{{ $assessment->recommendation }}</span>
                                            </div>
                                            @if($assessment->remarks)
                                                <div class="border-t pt-3">
                                                    <span class="text-xs text-gray-400 block uppercase font-bold mb-1">{{ __('Social Worker Remarks') }}</span>
                                                    <span class="font-semibold text-gray-700 leading-normal block">{{ $assessment->remarks }}</span>
                                                </div>
                                            @endif
                                        </div>

                                        <form method="POST" action="{{ route('approving-officer.review.decide', $item->id) }}" class="border-t pt-4">
                                            @csrf

                                            <div class="mb-4">
                                                <x-input-label for="approval_remarks_{{ $item->id }}" :value="__('Approval/Return Remarks')" class="font-bold text-gray-700" />
                                                <textarea id="approval_remarks_{{ $item->id }}" name="approval_remarks" rows="3" class="mt-1.5 block w-full" required>{{ old('approval_remarks') }}</textarea>
                                            ...
                                            <div class="flex justify-end gap-3 mt-6 border-t pt-4">
                                                ...
                                                <x-primary-button type="submit" name="decision" value="Approved" class="btn-primary">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="p-8 text-center text-gray-500 italic">{{ __('No applications pending review.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
